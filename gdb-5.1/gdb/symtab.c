/* gdb-5.1/gdb/symtab.c - core analysis suite
 *
 ...
 *
 * CVS: $Revision: 1.3 $ $Date: 2002/01/17 18:02:58 $
 */
...
static void get_member_offset(struct gnu_request *, struct type *);
static void dump_enum(struct type *, struct gnu_request *);
static void eval_enum(struct type *, struct gnu_request *);
static char *enum_tagname(struct type *);
...
gdb_get_datatype(struct gnu_request *req)
{
	 ...
	 */
	sym = lookup_symbol(req->name, 0, STRUCT_NAMESPACE, 0, 
		(struct symtab **) NULL);
	if (sym) {
                req->typecode = TYPE_CODE(sym->type);
                req->length = TYPE_LENGTH(sym->type);
		if (req->member)
			get_member_offset(req, sym->type);
 ...
 *  member field, and when found, return its offset.
 */
static void
get_member_offset(struct gnu_request *req, struct type *type)
{
	register short i;
	struct field *nextfield;
...
patch_load_module(struct objfile *objfile, struct minimal_symbol *msymbol)
{
	...
	if (msymbol) {
	        for (sp = lm->mod_load_symtable; 
			    ...
			    !STREQ(sp->name, SYMBOL_NAME(msymbol)))
				continue;
...
gdb_debug_command(struct gnu_request *req)
{
        ...
        if (sym) {
		fprintf_filtered(gdb_stdout, "%s sym: %lx\n", req->name, sym);
        }
#endif /* LOOKUP_DEBUG */
#ifdef ALPHA_TEST
	struct symbol *sym;

	if (gdb_print_args(req->pc, req->frame) == -1)
		req->flags |= GNU_COMMAND_FAILED;
#endif
}
#endif
