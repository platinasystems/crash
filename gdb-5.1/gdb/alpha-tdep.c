/* gdb-5.1/gdb/alpha-tdep.c - core analysis suite
 *
 ...
 *
 * CVS: $Revision: 1.3 $ $Date: 2001/12/04 20:15:01 $
 */
...
{
#ifdef MCLX
        extern CORE_ADDR gdb_starting_fp;
 
        CORE_ADDR last_known_frame = fi ? (CORE_ADDR)fi->frame :
                (CORE_ADDR)NULL; 
#endif
...
#ifdef MCLX
        if (fi)
                return((CORE_ADDR)fi->frame);
        else if (last_known_frame)
                return(last_known_frame);
        else
                return((CORE_ADDR)gdb_starting_fp);
#else
...
alpha_frame_chain (struct frame_info *frame)
{
  ...
  /* The previous frame from a sigtramp frame might be frameless
	...
	int gdb_MCLXDEBUG(ulong);
        struct symbol *sym;
        sym = PROC_SYMBOL(proc_desc);

        if (gdb_MCLXDEBUG(1))
            printf_filtered(" => SYMBOL_NAME: %s  PROC_FRAME_OFFSET: 0x%lx\n",
                SYMBOL_NAME(sym), 
...
#ifdef MCLX
int
get_frame_offset(CORE_ADDR pc)
{
	int gdb_MCLXDEBUG(ulong);
        alpha_extra_func_info_t proc_desc;
        struct symbol *sym;

        proc_desc = find_proc_desc (pc, 0);
        if (proc_desc) {
                sym = PROC_SYMBOL(proc_desc);
                if (gdb_MCLXDEBUG(1))
                    printf_filtered(
                    " => SYMBOL_NAME: %s  PROC_FRAME_OFFSET: 0x%lx\n",
                        sym ?  SYMBOL_NAME(sym) : "(no symbol found)",
                        PROC_FRAME_OFFSET(proc_desc));
                return(PROC_FRAME_OFFSET(proc_desc));
        } else {
                printf_filtered("proc_desc(%lx) failed!\n", pc);
                return 0;
        }
